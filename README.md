...
- An RxOn feature that shows initiative from our team is the ability to upload an image when adding a prescription. We did not want to store images directly in the database as this might reduce image quality and affect user experience, so we went with a cloud source service, AWS. The image is initially uploaded to the database Mongo collection, and then uploaded to AWS where it is deleted from the mongo collection. Meteor files allows us to secure our image upload link so it can’t be reverse engineered from the AWS client, ensuring optimal security for our project.
